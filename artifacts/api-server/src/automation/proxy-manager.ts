...
/**
 * Register a BRAND-NEW Cloudflare WARP identity and return it as a sing-box
 * WireGuard outbound. Each registration gets its own WireGuard key + assigned
 * addresses, which is how we obtain a different WARP exit IP on demand — no
 * `wgcf` binary needed (the key exchange is plain X25519, which Node can do,
 * and registration is a single HTTPS call).
 */
export async function registerWarpIdentity(): Promise<Record<string, unknown>> {
  // WireGuard keys are raw X25519. Node exports DER, so slice the trailing 32
  // raw bytes out of the SPKI/PKCS8 wrappers.
  ...
  if (!peer?.public_key || !addrs?.v4 || !data.config?.client_id) {
    throw new Error("WARP registration returned an unexpected payload");
  }
  // sing-box wants the 3-byte "reserved" derived from the base64 client_id.
  const cid = Buffer.from(data.config.client_id, "base64");
  const hostPort = peer.endpoint?.host ?? "engage.cloudflareclient.com:2408";
  const idx = hostPort.lastIndexOf(":");
  const host = idx > 0 ? hostPort.slice(0, idx) : hostPort;
  const port = idx > 0 ? parseInt(hostPort.slice(idx + 1), 10) : 2408;

  return {
    type: "wireguard",
    tag: "proxy",
    server: host,
    server_port: Number.isFinite(port) ? port : 2408,
    local_address: [`${addrs.v4}/32`, ...(addrs.v6 ? [`${addrs.v6}/128`] : [])],
    private_key: privB64,
    peer_public_key: peer.public_key,
    reserved: [cid[0] ?? 0, cid[1] ?? 0, cid[2] ?? 0],
    mtu: 1280,
  };
}
...
async function startSingBox(
  ...
  const outbound = await buildOutbound(type, link);
  ...
  // `child` is reassigned by rotate() — the process is replaced, but always on
  // the SAME listen_port, so `serverUrl` (and the browser's proxy) never changes.
  ...
  /**
   * Restart sing-box on the SAME port with a new outbound. The browser keeps
   * pointing at the unchanged local SOCKS address; only the exit IP changes.
   */
  const restartWith = async (nextOutbound: Record<string, unknown>): Promise<void> => {
    try { child.kill("SIGKILL"); } catch { /* ignore */ }
