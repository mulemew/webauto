  ...
  function scheduleRandomTask(taskId: number, windowMinutes: number, runsPerWindow: number): void {
    ...
    async function scheduleWindow(): Promise<void> {
      ...
      if (anchorMs > 0 && anchorMs <= now) {
        // Advance the anchor forward in windowMs steps until the window
        // that contains `now`.
        ...
        // No previous run — start the first window from now.
        windowStart = now;
...
export function scheduleTask(taskId: number, expression: string): void {
  ...
  if (parseAfterCompletionSchedule(expression) !== null) {
  ...
  try {
    // Dry-run: create without callback to validate, then stop immediately.
